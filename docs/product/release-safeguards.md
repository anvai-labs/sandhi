...
| SG05 | Integrate safe release checks into ordinary public CI | Unit/negative tests and workflow contracts green; independent adversarial review; focused develop PR and post-merge CI | In progress: local suite/review clean; [PR #237](https://github.com/anvai-labs/sandhi/pull/237) open, public CI pending |
| SG06 | Restrict release authority outside editable workflow code | Read-back evidence for approved tag/environment controls; preserve current protections; identify repository-secret exposure | Pending: exact remote changes require scoped approval |
...
Remote CI, owner authority controls and final release approval remain explicitly open. No
tag, environment/ruleset mutation, package upload or production deployment has been performed
as part of this implementation checkpoint.
