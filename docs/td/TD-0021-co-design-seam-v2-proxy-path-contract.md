...
| **P4** | D1 + D2 + D3 — idempotent metering — **DONE** (2026-09-02, review-corrected): `idempotency_dedup` ledger-co-durable with the lease TTL, keyed `(vkey, idempotency-key)` (R1); settle-then-record under one lock; reclaim sweep prunes expired rows (not prune-on-sight only); replay/fallback counters; the volatile arm counts (D3). **Semantics, stated exactly: the METER counts logical calls (a repeat's duplicate event is dropped); ENFORCEMENT counts physical calls (the retry's lease settles into spent — it really consumed tokens).** Both counters visible in /metrics. The sharded two-shard window is documented as a TD-0016 P2+ tripwire on the dedup methods | Two requests with the same `(vkey, idempotency-key)` inside the window produce **one usage event** (the meter counts the logical call; BOTH physical leases still reserve and settle — enforcement counts physical calls); outside the window, two events; with dedup unavailable, two events (D3's fail-toward-counting), and the fallback is counted in a metric (now `sandhi_idempotent_fallbacks_total`; the replay is `sandhi_idempotent_replays_total`) |
