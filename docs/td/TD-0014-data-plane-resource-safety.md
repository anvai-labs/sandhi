...
| **P4** | D5 — per-tenant bulkheads + sharded limiter. **Design notes from P1–P3:** the bulkhead slots belong on the same accept/ingress path as `ConnectionPolicy` (reuse `ingress_routes` + the `ConnectionPolicy` struct, not a parallel mechanism), and `ClientAddr` (P3's per-request middleware output) is the natural per-IP key — its consumer arrives here, removing its `#[allow(dead_code)]` | Tenant A saturating its share leaves tenant B's admission latency within a recorded bound (fairness measured, not asserted qualitatively); the sharded limiter reproduces every TD-0012 P1 semantic test unchanged |
