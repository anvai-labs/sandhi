...
- The proxy is **two-plane** (ADR-0004 D1), and both planes are wired. **Transparent plane:** when `ingress_family(dialect) == provider.family()` and the provider exposes a `raw_forwarder()`, the request is forwarded **byte-exact** and metered in flight (`transparent_stream_response` / `transparent_complete_response`, `sandhi-proxy/src/lib.rs`), built on the O(1) `metered_passthrough` primitive at the *adapter* layer (`sandhi-providers/src/lib.rs`). **Translation plane:** a cross-family request still decodes to `ChatRequestV1` and re-encodes, which can drop provider-specific extras. There is **no Gemini/Cohere ingress dialect** yet — only `/v1/chat/completions`, `/v1/messages`, `/v1/responses` — so those two are reachable as *upstreams* only, always via the translation plane.
